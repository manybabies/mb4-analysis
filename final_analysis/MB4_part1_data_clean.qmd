...
```{r check_reliability}
library(data.table)
clean_reliability_looking_data <- read_csv(here("main_data", "lookingtime_reliability_data.csv"))
...
participant_summaries |> 
  summarise(across(starts_with("ex_"), \(x) 100*sum(x, na.rm = TRUE)/nrow(participant_summaries))) |> 
  t()
```
...
# valid sample size in two conditions
```{r valid sample size in two conditions}
sum(primary_data$condition=="social")
sum(primary_data$condition=="nonsocial")
```

# sample size for each lab (with valid data)
```{r sample size for each lab (with valid data)}
valid_lab <- subset(summary_by_lab,!is.na(mean_age))
mean(valid_lab$valid_sample_size)
sd(valid_lab$valid_sample_size)
min(valid_lab$valid_sample_size)
max(valid_lab$valid_sample_size)
```
