...
Check for fit again
```{r model checking LT}
pp_check(brm.noninfo.total_looking)
ppc_stat(y = primary_data$chose_helper, 
         yrep = posterior_predict(brm.noninfo.total_looking, draws = 200), 
         stat="num_zeros") #function from above.
```
...
df <- brm.noninfo.total_looking %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.total_looking.csv", row.names = FALSE)
...
Check the fit.
```{r model checking sep looking time}
pp_check(brm.noninfo.looking)
ppc_stat(y = primary_data$chose_helper, 
         yrep = posterior_predict(brm.noninfo.looking, draws = 200), 
         stat="num_zeros") #function from above.
```
...
df <- brm.noninfo.looking %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.looking.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.hab_trial_num %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.hab_trial_num.csv", row.names = FALSE)

```

Check fit again
```{r model checking habitution}
pp_check(brm.noninfo.hab_trial_num)
ppc_stat(y = primary_data$chose_helper, 
         yrep = posterior_predict(brm.noninfo.hab_trial_num, draws = 200), 
         stat="num_zeros") #function from above.
```

### dates (the number of days between the median test date and the earliest one)

```{r dates full_model, results=FALSE, message=FALSE}
dates_data <- read.csv(here("main_data", "laboratory_testing_log.csv"))

primary_data_dates <- merge(primary_data,dates_data,by ="lab_id")
...
brm.noninfo.dates <- brm(chose_helper ~ 1 + condition + scale(dates) + condition:scale(dates) +
                       (1 + condition + scale(dates) + condition:scale(dates) | lab_id), 
                        family = bernoulli, data = primary_data_dates,
                        prior = priors.noninformative, iter = 10000, 
                        control = list(adapt_delta = .99, max_treedepth = 20),
                        chains = 4, future = T, save_all_pars = TRUE)
```
...
```{r nodates model}
brm.noninfo.dates_nodates <- brm(chose_helper ~ 1 + condition+ 
                       (1 + condition| lab_id), 
                        family = bernoulli, data = primary_data_dates,
                        prior = priors.noninformative, iter = 10000, 
                        control = list(adapt_delta = .99, max_treedepth = 20),
...
brm.noninfo.dates_nodatescondition <- brm(chose_helper ~ 1 + condition + scale(dates) +
                       (1 + condition + scale(dates)| lab_id), 
                        family = bernoulli, data = primary_data_dates,
                        prior = priors.noninformative, iter = 10000, 
                        control = list(adapt_delta = .99, max_treedepth = 20),
                        chains = 4, future = T, save_all_pars = TRUE)
...
```{r bayes_nodates}
# Bridge-sample posterior
bridge.noninfo.dates <- bridge_sampler(brm.noninfo.dates, silent = T)
bridge.noninfo.no_dates <- bridge_sampler(brm.noninfo.dates_nodates, silent = T)
# Compute Bayes factors
bf.noninfo.dates <- bf(bridge.noninfo.dates, bridge.noninfo.no_dates)
print(bf.noninfo.dates)

# Bridge-sample posterior
bridge.noninfo.no_datescondition <- bridge_sampler(brm.noninfo.dates_nodatescondition, silent = T)
# Compute Bayes factors
bf.noninfo.datescondition <- bf(bridge.noninfo.dates, bridge.noninfo.no_datescondition)
print(bf.noninfo.datescondition)

```



```{r dates full_estimates of each parameter, message=FALSE}
brm.noninfo.dates %>% estimates.brm_fixef(prob = .95) %>%
  kable()

# Convert to data frame if necessary
df <- brm.noninfo.dates %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.dates.csv", row.names = FALSE)

```
```{r plot dates and effect}
aggregated_data <- primary_data_dates %>%
  group_by(lab_id, condition) %>%
  summarize(
    dates = mean(dates),
    chose_helper = mean(chose_helper),
    .groups = 'drop'
  )

scatter <- ggplot(aggregated_data,
                  aes(x = dates,
                      y = chose_helper,
                      colour = condition)) +
  stat_smooth(method = "lm") +
  geom_point(position = position_jitter(height = .05, width = 0),
             size = 1) +
  xlab("Dates (median)") + ylab("Choice") + theme(legend.position = "top") +
  scale_colour_brewer(palette = "Dark2", name = "Condition",
                      breaks = c("nonsocial", "social"),
                      labels = c("non-social", "social")) +
  scale_y_continuous(breaks = c(0, 1),
                     labels = c("Hinderer/\nPush-down character", "Helper/\nPush-up character"))+ 
  scale_x_continuous(breaks = c(0,100,200,300),labels=c("11/5/2022","2/13/2023","5/24/2023","9/1/2023"))+ 
  geom_hline(yintercept = 0.5, linetype = "dashed", col = "black", lty = 2)
ggsave("dates_scatter.png", scatter,
       units = "mm", width = 180, height = 100, dpi = 1000)
(scatter)
...
# Convert to data frame if necessary
df <- brm.noninfo.touch_both %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.touch_both.csv", row.names = FALSE)

```

```{r model checking touch both}
pp_check(brm.noninfo.touch_both)
ppc_stat(y = primary_data$chose_helper, 
         yrep = posterior_predict(brm.noninfo.touch_both, draws = 200), 
         stat="num_zeros") #function from above.
```
...
# Convert to data frame if necessary
df <- brm.noninfo.RA_1_naive %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.RA_1_naive.csv", row.names = FALSE)

```

```{r model checking naive}
pp_check(brm.noninfo.RA_1_naive)
ppc_stat(y = primary_data$chose_helper, 
         yrep = posterior_predict(brm.noninfo.RA_1_naive, draws = 200), 
         stat="num_zeros") #function from above.
```
...
# Convert to data frame if necessary
df <- brm.noninfo.push_up_order %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.push_up_order.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.push_up_identity %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.push_up_identity.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.push_up_side %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.push_up_side.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.visual_angle %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.visual_angle.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.second_session %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.second_session.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.choice_experimenter_mask %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.choice_experimenter_mask.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.method %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.method.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.infant_handedness %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.infant_handedness.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.colorblindness_primaryfamily %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.colorblindness_primaryfamily.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.participant_gender %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.participant_gender.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.percent_primarylanguage %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.percent_primarylanguage.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.exclusion_rate %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.exclusion_rate.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.choice_exclusion_rate %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.choice_exclusion_rate.csv", row.names = FALSE)
...
```{r habituation effect centered condition}
...
# Convert to data frame if necessary
df <- brm.noninfo.looking.full %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.looking.full.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.looking.full.young %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.looking.full.young.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.looking.full.old %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.looking.full.old.csv", row.names = FALSE)
...
# Convert to data frame if necessary
df <- brm.noninfo.looking.full.social %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.looking.full.social.csv", row.names = FALSE)
...
# Extract the data frame
df <- brm.noninfo.looking.full.nonsocial %>% estimates.brm_fixef(prob = .95)

# Save the data frame as a CSV file
write.csv(df, "brm.noninfo.looking.full.nonsocial.csv", row.names = FALSE)
...
scatter <- ggplot(primary_data,
  ...
  scale_colour_brewer(palette = "Dark2", name = "Condition",
                      breaks = c("nonsocial", "social"),
                      labels = c("non-social", "social")) 
ggsave("age_scatter_looking.png", scatter,
       units = "mm", width = 180, height = 100, dpi = 1000)
(scatter)
```
...
scatter.bayes <- ggplot(primary_data,
  ...
  scale_fill_brewer(palette = "Dark2", name = "Condition",
                   breaks = c("nonsocial","social"),
                      labels = c("non-social","social")) 
ggsave("age_scatter_bayes_looking.png", scatter.bayes,
       units = "mm", width = 180, height = 100, dpi = 1000)
(scatter.bayes)
...
# big model
```{r a model including all moderators}
priors.noninformative <- set_prior("student_t(3, 0, 2)",
                                   class = "sd")


brm.noninfo.all <- brm(chose_helper ~ 1 + condition+ z_age_days + condition:z_age_days + total_looking + condition:total_looking + habituation_trial + condition:habituation_trial+ scale(dates) + condition:scale(dates) + touch_both + condition:touch_both+ RA_1_naive + condition:RA_1_naive+ push_up_order + condition:push_up_order+ push_up_identity + condition:push_up_identity+ push_up_side + condition:push_up_side+ visual_angle + condition:visual_angle+ second_session + condition:second_session+ choice_experimenter_mask + condition:choice_experimenter_mask+ method + condition:method+ infant_handedness + condition:infant_handedness+ colorblindness_primaryfamily + condition:colorblindness_primaryfamily+ participant_gender + condition:participant_gender+ percent_primarylanguage + condition:percent_primarylanguage+ exclusion_eligible + condition:exclusion_eligible+ choice_exclusion_rate + condition:choice_exclusion_rate+ habituation + condition:habituation+
                       (1 | lab_id), 
                        family = bernoulli, data = primary_data_dates,
                        prior = priors.noninformative, iter = 10000, 
                        control = list(adapt_delta = .99, max_treedepth = 20),
...
```
```{r}
brm.noninfo.all %>% estimates.brm_fixef(prob = .95) %>%
  kable()
