...
```{r}
scatter <- ggplot(all_habituation_data, 
                  aes(x = age_months, y = choice, group = 1)) +
  stat_smooth(method = "lm", size = .5) +
...
bayes_mod <- brm(chose_helper_num ~ 1 + z_age_days + (1 + z_age_days | lab_id), 
                 family = bernoulli, data = all_habituation_data, 
                 iter = 10000, 
                 prior =  priors.full,
                 control = list(adapt_delta = .99, max_treedepth = 20),
                 chains = 4, future = T, save_all_pars = TRUE)
bayes_mod_nointercept <- brm(chose_helper_num ~ 0 + z_age_days + 
                               (0 + z_age_days | lab_id),
                             family = bernoulli, data = all_habituation_data, 
                             iter = 10000,
...
```{r}
bf10.intercept <- bf(H1.bridge, H0_nointercept.bridge)
print(paste("Intercept BF:", bf10.intercept$bf))
```

Bayes factor for age.

```{r}
bf10.age <- bf(H1.bridge, H0_noage.bridge)
print(paste("Age BF:", bf10.age$bf))
```
...
all_habituation_data.marginal_plot <- ggplot(all_habituation_data,
                                             aes(x = z_age_days,
                                                 y = chose_helper_num,
                                                 group = 1)) +
...
(all_habituation_data.marginal_plot)
```
...
```{r results='hide', message=FALSE}
bayes_mod2 <- brm(chose_helper_num ~ z_age_days + (z_age_days | lab_id), 
...
bayes_mod2_nointercept <- brm(chose_helper_num ~ z_age_days - 1 + 
                                (z_age_days - 1 | lab_id),
                              family = bernoulli, data = habituators_data, 
                              iter = 10000,
                              prior =  priors.nointercept,
                              control = list(adapt_delta = .99, max_treedepth = 20), 
                              chains = 4, future = T, save_all_pars = TRUE)
bayes_mod2_noage <- brm(chose_helper_num ~ 1 + 
                          (1 | lab_id),
                        family = bernoulli, data = habituators_data, 
                        iter = 10000,
                        prior =  priors.noage,
...
```{r}
H0_nointercept2.bridge <- bridge_sampler(bayes_mod2_nointercept, silent = TRUE)
H0_noage2.bridge <- bridge_sampler(bayes_mod2_noage, silent = TRUE)
H1.bridge2 <- bridge_sampler(bayes_mod2, silent = TRUE)
```

Bayes factor for intercept.

```{r}
bf10.intercept2 <- bf(H1.bridge2, H0_nointercept2.bridge)
print(paste("Intercept BF:", bf10.intercept2$bf))
```

Bayes factor for age.

```{r}
bf10.age2 <- bf(H1.bridge2, H0_noage2.bridge)
print(paste("Age BF:", bf10.age2$bf))
```

Summary of the full model

```{r}
bayes_mod2 %>% estimates.brm_fixef(prob = .95) %>%
  kable()
```
...
habituators_data.marginal_plot <- ggplot(habituators_data,
                                         aes(x = z_age_days,
                                             y = chose_helper_num,
                                             group = 1)) +
  ...
  scale_y_continuous(name = "Choice",
                     breaks = c(0, 1),
                     labels = c("Hinderer", "Helper"))
(habituators_data.marginal_plot)
```
