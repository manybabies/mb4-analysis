...
author: "Kelsey Lucca, Arthur Capelier-Mourguy, Mike Frank, Yiyi Wang, & Francis Yuen" (needs to update)
date: "02/16/2024"
output:
...
add page number in ms

# Introduction

In this document, we conduct and document the pre-registered analyses and exploratory analyses for the MB4 study.

```{r load packages and functions required for analyses, message=FALSE}
# packages
library(tidyverse) 
#library(brms)
library(binom)
...
looking_reliability <- setDF(data.table::melt(setDT(clean_reliability_looking_data), 
                                  id.vars = c("lab_id","subj_id","subj_num"), 
                                  measure.vars = list(c("trial1_lookingtime_freezeframe","trial2_lookingtime_freezeframe","trial3_lookingtime_freezeframe","trial4_lookingtime_freezeframe","trial5_lookingtime_freezeframe","trial6_lookingtime_freezeframe","trial7_lookingtime_freezeframe","trial8_lookingtime_freezeframe","trial9_lookingtime_freezeframe","trial10_lookingtime_freezeframe","trial11_lookingtime_freezeframe","trial12_lookingtime_freezeframe","trial13_lookingtime_freezeframe","trial14_lookingtime_freezeframe")), 
                                  value.name = c("rating2")))
looking_reliability <- plyr::rename(looking_reliability, c(variable="trial"))

looking <- setDF(data.table::melt(setDT(clean_data), 
                                  id.vars = c("lab_id","subj_id","subj_num"), 
                                  measure.vars = list(c("trial1_lookingtime_freezeframe","trial2_lookingtime_freezeframe","trial3_lookingtime_freezeframe","trial4_lookingtime_freezeframe","trial5_lookingtime_freezeframe","trial6_lookingtime_freezeframe","trial7_lookingtime_freezeframe","trial8_lookingtime_freezeframe","trial9_lookingtime_freezeframe","trial10_lookingtime_freezeframe","trial11_lookingtime_freezeframe","trial12_lookingtime_freezeframe","trial13_lookingtime_freezeframe","trial14_lookingtime_freezeframe")), 
                                  value.name = c("rating1")))
looking <- plyr::rename(looking, c(variable="trial"))

looking_merge <- merge(looking_reliability,looking,by = c("lab_id","subj_id","subj_num","trial"))


data_reliability <- looking_merge[, c("rating1", "rating2")]
data_reliability <- na.omit(data_reliability)
# Calculate ICC using irr package (NAs are automaticlaly ignored)
icc_result <- icc(data_reliability, model = "oneway", type = "agreement", unit = "single")
print(icc_result)


```

# looking time calculation for later analyses
...
```{r prepare data for looking time calculation, message=FALSE}
num <- nrow(clean_data)
...
## experimenter error (based on looking time data) a

```{r examine experimenter error based on looking time data, message=FALSE}
clean_data$experimenter_error = 0 # 0 indicates no exclusion, 1 indicates exclusion due to experimenter error of presenting stimuli incorrectly
...
for (j in 1:num)
{
   ...
   for (i in 1:14)
   {
     if (!is.na(data[j,4*i-3])&!is.na(data[j,4*i]))
     if (data[j,4*i-3] == "N" & data[j,4*i] != "2")
     {
       clean_data$experimenter_error[j] =1
       break
       }
   }
}
...
# Tally
experimentererror_tally <- clean_data %>%
  group_by(experimenter_error) %>%
  summarize(n = n())
print(experimentererror_tally)
```

## Missing critical period

First, we calculate the number of participants excluded because labs report they missed at least one critical period for 3 consecutive times (after excluding experimenter error).
  ...
  
{
  for (k in 1:4)
  {
   if (!is.na(data[j,4*k-1]) & !is.na(data[j,4*(k+1)-1]) & !is.na(data[j,4*(k+2)-1])& (as.numeric(data[j,4*k-1])+as.numeric(data[j,4*(k+1)-1])+as.numeric(data[j,4*(k+2)-1]))>=12)
     {
...
clean_data_tmp <- clean_data_tmp %>%
  mutate(habituation = ifelse(clean_data_tmp$ex_habcrt == 1,
                              "not_interpretable",
                              clean_data_tmp$habituation),
         habcrt_value = ifelse(clean_data_tmp$ex_habcrt == 1,
                              "not_interpretable",
                              clean_data_tmp$habcrt_value),
         num_habcrt = ifelse(clean_data_tmp$ex_habcrt == 1,
                              "not_interpretable",
                              clean_data_tmp$num_habcrt),
         num_hab = ifelse(clean_data_tmp$ex_habcrt == 1,
                              "not_interpretable",
                              clean_data_tmp$num_hab),
         num_should = ifelse(clean_data_tmp$ex_habcrt == 1,
                             "not_interpretable",
                             clean_data_tmp$num_should))

clean_data_tmp <- clean_data_tmp %>%
  mutate(habituation = ifelse(clean_data_tmp$experimenter_error == 1,
                              "not_interpretable",
                              clean_data_tmp$habituation),
         habcrt_value = ifelse(clean_data_tmp$experimenter_error == 1,
                              "not_interpretable",
                              clean_data_tmp$habcrt_value),
         num_habcrt = ifelse(clean_data_tmp$experimenter_error == 1,
                              "not_interpretable",
                              clean_data_tmp$num_habcrt),
         num_hab = ifelse(clean_data_tmp$experimenter_error == 1,
                              "not_interpretable",
                              clean_data_tmp$num_hab),
         num_should = ifelse(clean_data_tmp$experimenter_error == 1,
                             "not_interpretable",
                             clean_data_tmp$num_should))

```

```{r remove temperary data}
...
data_check_hab6 <- data_check_hab6 %>%
  mutate(sum_4to6 = trial4_lookingtime_freezeframe + trial5_lookingtime_freezeframe + trial6_lookingtime_freezeframe,
         habituated_at_6 = ifelse(sum_4to6 <= as.numeric(habcrt_value), "yes", "no"),
         .after = habcrt_value)
...
data_check_hab14 <- data_check_hab14 %>%
  mutate(sum_12to14 = trial12_lookingtime_freezeframe + trial13_lookingtime_freezeframe + trial14_lookingtime_freezeframe,
         habituated_at_14 = ifelse(sum_12to14 <= as.numeric(habcrt_value), 1, 0),
  ...
  
{
  for (k in 1:14)
  {
    if (is.na(data[j,4*k-1]) & is.na(data[j,4*k-2]))
    {
...
```
# check the overlap with equipment error reported by labs

```{r calculate error number only due to phyhab error (equipment error)}
error <- subset(clean_data,ex_error==1 & ex_critical==0 & ex_habcrt==0 & experimenter_error==0)
nrow(error)

```

## number for each type of exclusion 
...
````{r exclusion based on lab report, message=FALSE}
# 1 indicates "excluded", 0 indicates "non-excluded"

clean_data$ex_Preterm <- ifelse((clean_data$exclusion_type_1 == "Preterm"|clean_data$exclusion_type_2 == "Preterm"|clean_data$exclusion_type_3 == "Preterm"|clean_data$exclusion_type_4 == "Preterm"),1,0)
print(paste0("Preterm:",sum(clean_data$ex_Preterm,na.rm = TRUE)))
print(sum(clean_data$ex_Preterm,na.rm = TRUE)/nrow(clean_data))

clean_data$ex_DevDisorder <- ifelse((clean_data$exclusion_type_1 == "DevDisorder"|clean_data$exclusion_type_2 == "DevDisorder"|clean_data$exclusion_type_3 == "DevDisorder"|clean_data$exclusion_type_4 == "DevDisorder"),1,0)
print(paste0("DevDisorder:",sum(clean_data$ex_DevDisorder,na.rm = TRUE)))
print(sum(clean_data$ex_DevDisorder,na.rm = TRUE)/nrow(clean_data))

clean_data$ex_FailToMakeChoice <- ifelse((clean_data$exclusion_type_1 == "FailToMakeChoice"|clean_data$exclusion_type_2 == "FailToMakeChoice"|clean_data$exclusion_type_3 == "FailToMakeChoice"|clean_data$exclusion_type_4 == "FailToMakeChoice"),1,0)
print(paste0("FailToMakeChoice:",sum(clean_data$ex_FailToMakeChoice,na.rm = TRUE)))
print(sum(clean_data$ex_FailToMakeChoice,na.rm = TRUE)/nrow(clean_data))

clean_data$ex_OutsideInterference <- ifelse((clean_data$exclusion_type_1 == "OutsideInterference"|clean_data$exclusion_type_2 == "OutsideInterference"|clean_data$exclusion_type_3 == "OutsideInterference"|clean_data$exclusion_type_4 == "OutsideInterference"),1,0)
print(paste0("OutsideInterference:",sum(clean_data$ex_OutsideInterference,na.rm = TRUE)))
print(sum(clean_data$ex_OutsideInterference,na.rm = TRUE)/nrow(clean_data))

clean_data$ex_ExperimenterError <- ifelse((clean_data$exclusion_type_1 == "ExperimenterError"|clean_data$exclusion_type_2 == "ExperimenterError"|clean_data$exclusion_type_3 == "ExperimenterError"|clean_data$exclusion_type_4 == "ExperimenterError"),1,0)
clean_data$ex_ExperimenterError <- ifelse(clean_data$experimenter_error==1,1,clean_data$ex_ExperimenterError)
#incorporate experimenter error based on looking time
print(paste0("ExperimenterError:",sum(clean_data$ex_ExperimenterError,na.rm = TRUE)))
print(sum(clean_data$ex_ExperimenterError,na.rm = TRUE)/nrow(clean_data))

clean_data$ex_EquipmentError <- ifelse((clean_data$exclusion_type_1 == "EquipmentError"|clean_data$exclusion_type_2 == "EquipmentError"|clean_data$exclusion_type_3 == "EquipmentError"|clean_data$exclusion_type_4 == "EquipmentError"),1,0)
clean_data$ex_EquipmentError <- ifelse(clean_data$ex_error==1,1,clean_data$ex_EquipmentError)
#incorporate pyhab error
print(paste0("EquipmentError:",sum(clean_data$ex_EquipmentError,na.rm = TRUE)))
print(sum(clean_data$ex_EquipmentError,na.rm = TRUE)/nrow(clean_data))

clean_data$ex_FussOut <- ifelse((clean_data$exclusion_type_1 == "FussOut"|clean_data$exclusion_type_2 == "FussOut"|clean_data$exclusion_type_3 == "FussOut"|clean_data$exclusion_type_4 == "FussOut"),1,0)
print(paste0("FussOut:",sum(clean_data$ex_FussOut,na.rm = TRUE)))
print(sum(clean_data$ex_FussOut,na.rm = TRUE)/nrow(clean_data))

count_FailToViewCritPeriod <- sum(clean_data$ex_critical, na.rm = TRUE)
print(paste0("FailToViewCritPeriod:",count_FailToViewCritPeriod))
print(count_FailToViewCritPeriod/nrow(clean_data))

count_FailToSetHabCrit <- sum(clean_data$ex_habcrt, na.rm = TRUE)
print(paste0("FailToSetHabCrit:",count_FailToSetHabCrit))
print(count_FailToSetHabCrit/nrow(clean_data))

```

## new exclusion column including all types

```{r exclusion_all column}
clean_data$exclusion <- ifelse(clean_data$ex_DevDisorder==1 | clean_data$ex_Preterm==1 | clean_data$ex_FailToMakeChoice==1 | clean_data$ex_OutsideInterference==1 |  clean_data$ex_ExperimenterError==1 | clean_data$ex_EquipmentError==1 | clean_data$ex_FussOut==1 |clean_data$ex_critical==1 | clean_data$ex_habcrt==1,"excluded",NA)

clean_data$exclusion <- ifelse(is.na(clean_data$exclusion),"not_excluded","excluded")

tally_exclusion_all <- clean_data %>%
  group_by(exclusion) %>%
  summarize(n = n())
print(tally_exclusion_all)
...
```{r checking exclusion by lab id, message = FALSE}
exclusion_by_lab <- clean_data %>%
  group_by(lab_id) %>%
  count(exclusion) %>%
  pivot_wider(names_from = exclusion,
              values_from = n) %>%
  replace(is.na(.), 0) %>%
  mutate(proportion_excluded = excluded / (excluded + not_excluded)) 

asu <- filter(clean_data, lab_id == "emlasu") %>%
  filter(ex_habcrt == 1)
# Three labs have higher than 75% exclusion. Closer inspection to see what happened
# Ignore UIWCARL since n = 3

high_ex_labs <- clean_data %>%
  filter(lab_id == "biccbabylab" | lab_id == "mecdmpihcbs",
         exclusion == "excluded") %>%
  group_by(lab_id, ex_habcrt) %>%
  summarize(n = n())

# extremely high exclusion rates because hab criteria never set
...
# convert clean_data to data frame so that function below can work
clean_data <- as.data.frame(clean_data)
...
lt_duplicates <- lt_matrix %>%
  ...
  rename(trial1_lookingtime_freezeframe = V1) # no duplicates #
...
### First with glmmTMB (Nico)

```{r load packages, message=FALSE}
# should be move at the beginning of the script once stable
library(glmmTMB)
library(DHARMa)
```

### FULL MODEL as specified by Yiyi

```{r full_preregistered_model_glmmTMB message=FALSE}
# Define and summarize model - note that we removed correlations between random effects for convergence
full_model <- glmmTMB(chose_helper ~ 1 + condition + z_age_days + 
                  condition:z_age_days + 
                  (1 + condition + z_age_days + condition:z_age_days || lab_id), 
                data = primary_data, family = binomial)
summary(full_model)
...
```{r full_model_diagnostic_glmmTMB message=FALSE}
full_model_sim <- simulateResiduals(full_model)
plot(full_model_sim) # No problems found

# Plot the results for visual inspection 
plot(ggpredict(full_model, c("z_age_days[all]", "condition")))
```

### REDUCED MODEL

```{r reduced_model_glmmTMB message=FALSE}
# Define and summarize model
reduced_model <- glmmTMB(chose_helper ~ 1 + condition + z_age_days + 
                     (1 + condition + z_age_days + condition:z_age_days || lab_id), 
                   data = primary_data, family = binomial)
...
# Run model diagnostics using DHARMa
# reduced_model_sim <- simulateResiduals(reduced_model)
# plot(reduced_model_sim) # No problems found

# Plot the results for visual inspection
# plot(ggpredict(reduced_model, c("z_age_days[all]", "condition")))
```

### LANGUAGE MODEL

NOTE FOR JULIEN AND NICO:

- no intercept in model? i.e. ~ 1 + ....
- you included interaction of age and condition in this model, but Yiyi did not. Should we keep consistent?

```{r language_model_glmmTMB message=FALSE}
# Define and summarize model
language_model <- glmmTMB(chose_helper ~ z_age_days * condition + 
          percent_primarylanguage + percent_primarylanguage:condition + 
          (1 + condition + z_age_days + condition:z_age_days || lab_id), data = primary_data, family = binomial)
summary(language_model)
# No significant effects

# Run model diagnostics using DHARMa
# language_model_sim <- simulateResiduals(language_model)
# plot(language_model_sim) # No problems found
```

### Plot the results for visual inspection

```{r language_model__plot_glmmTMB message=FALSE}

plot(ggpredict(language_model, c("z_age_days[all]", "percent_primarylanguage[50, 75, 100]", "condition")))
```


### Now with lme4 (Julien)
...
### FULL MODEL as specified by Yiyi (lme4 version)

```{r full_preregistered_model_lme4 message=FALSE}
# Define and summarize model - note that we can keep correlations between random effects here with lme4
full_model_lme4 <- glmer(chose_helper ~ 1 + condition + z_age_days + 
                  condition:z_age_days + 
                  (1 + condition + z_age_days + condition:z_age_days | lab_id), 
                data = primary_data, family = binomial(link = "logit"),
                control=glmerControl(optimizer="bobyqa",
                                 optCtrl=list(maxfun=2e5)))
summary(full_model_lme4)
# No significant effects
```
...
```{r full_model_diagnostic_glmmTMB message=FALSE}
full_model_sim_lme4 <- simulateResiduals(full_model_lme4)
plot(full_model_sim_lme4) # No problems found

# Plot the results for visual inspection 
plot(ggpredict(full_model_lme4, c("z_age_days[all]", "condition")))
```

### REDUCED MODEL - LME4

NOTE FOR NICO AND JULIEN: 
- Why does the reduced model still have condition*age as a random effect? Is it typo or intentional?

```{r reduced_model_lme4 message=FALSE}
# Define and summarize model - note that we can keep correlations between random effects here with lme4
reduced_model_lme4 <- glmer(chose_helper ~ 1 + condition + z_age_days + 
                  (1 + condition + z_age_days + condition:z_age_days | lab_id), 
                data = primary_data, family = binomial(link = "logit"),
                control=glmerControl(optimizer="bobyqa",
                                 optCtrl=list(maxfun=2e5)))
summary(reduced_model_lme4)
# No significant effects
```
