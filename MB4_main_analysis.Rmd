...
# num_hab: the number of trials that infants should see (when habituation was reached or 14 when not habituated)
  ...
  
{
  ...
  numhabcrt <- clean_data$num_habcrt[j]
...
clean_data$num_hab <- ifelse(clean_data$ex_critical==0 # if infants watched all critical periods
                             & clean_data$ex_habcrt ==0 # and successfully set a habituation criteria
                             & clean_data$num_hab == 0 # and they have 0 as their habituation number (i.e. unchanged after loop)
                             & clean_data$habituation == 0, # and they did not habituate
                             14, # set their num_hab to 14, since they should have saw the maximum number of trials
                             clean_data$num_hab) # Otherwise, other cases are fine and num_hab should be calculated as above
...
clean_data$ex_error <- ifelse((clean_data$num_hab == clean_data$num_see)| # num_hab matches num_see, or
                                (clean_data$num_see<6) | # infants saw less than the minimum of 6 trials, indicating other types of error, or
                                is.na(clean_data$num_hab)  # num_hab is NA, meaning infant was excluded above 
                              ,0,1)
...
primary_data$hab <- ifelse(primary_data$hab_trial_num==15,0,1) ## This line of code isn't working but not sure why - Francis
```
